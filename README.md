...
- **CMS Sync Protocol Migration:** Apply `supabase/migrations/202607080001_add_cms_sync_protocol.sql` to store snapshot audit metadata (`content_hash`, `payload_bytes`) and webhook event logs (`cms_sync_events`).
...
- **Notion CMS Webhook (`/api/notion/cms-webhook`):** Configure this as the Notion webhook URL for CMS/database/page events. The endpoint supports Notion's initial `verification_token`, validates the `X-Notion-Signature` HMAC using `NOTION_CMS_WEBHOOK_SECRET` (fallback: `NOTION_CMS_WEBHOOK_VERIFICATION_TOKEN`, then `NOTION_WEBHOOK_VERIFICATION_TOKEN`), records each event in `cms_sync_events`, then immediately refreshes the Supabase snapshot and Next.js cache for every CMS page path.
- **CMS Live Refresh (`/api/notion/cms-status` + `CmsLiveRefresh`):** Public pages poll lightweight snapshot metadata while the tab is visible. When the snapshot hash changes after a webhook or manual sync, the client calls `router.refresh()` so open production tabs pick up CMS edits without a manual browser refresh.
- **Scheduled Reconciliation:** On Vercel Hobby, Cron calls `/api/notion/sync-cms` once daily. This is only the fallback for missed webhook deliveries; immediate CMS updates should flow through the Notion webhook and live page refresh. For a sub-daily fallback cadence, use Vercel Pro or an external scheduler.
- **Persistent Notion Image Cache:** `/api/notion-image` now checks Supabase Storage before fetching Notion/S3 signed image URLs. Create a Storage bucket named `notion-images` or set `SUPABASE_NOTION_IMAGE_BUCKET`; cached objects use a one-year Storage cache header while browser responses use `stale-while-revalidate`.
- **Secure Revalidation Controller (`/api/notion/revalidate`):** A public, rate-limited endpoint that syncs the CMS snapshot, then clears and revalidates all CMS caches (`events`, `beranda`, `profil`, `kkm`, `faq`, `redirects`) plus every CMS page slug without exposing integration keys. Includes an in-memory **10-second rate-limiting cooldown** to protect Notion API rates.
